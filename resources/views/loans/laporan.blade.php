...
@section('title', 'Laporan Pinjaman - ADMS')
@section('page-title', 'Laporan Pinjaman')
@section('page-subtitle', 'Laporan pinjaman dan pembayaran bulanan')
...
<div class="space-y-6">
    ...
    <div class="bg-white rounded-xl shadow-sm p-6">
        <form method="GET" action="{{ route('loans.laporan') }}" class="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div>
    ...
    <div class="bg-white rounded-xl shadow-sm">
        <div class="px-6 py-4 border-b border-gray-200">
            <h3 class="text-lg font-semibold text-gray-800">
                ...
                Laporan Pinjaman Bulan {{ $reportMonthName }} {{ $year }}
            </h3>
        </div>
                ...
                <tbody class="divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-6 py-4 text-right text-sm text-gray-700">
                            @if($row['sisa_before'] > 0)
                                <span class="text-yellow-600">Rp {{ number_format($row['sisa_before'], 0, ',', '.') }}</span>
                            @else
                        ...
                        <td class="px-6 py-4 text-right text-sm text-gray-700">
                            @if($row['bayar_month'] > 0)
                                <span class="text-green-600">Rp {{ number_format($row['bayar_month'], 0, ',', '.') }}</span>
                            @else
                                <span class="text-gray-400">-</span>
                            @endif
                        </td>
                        <td class="px-6 py-4 text-right text-sm font-medium">
                            @if($row['sisa_akhir'] > 0)
                                <span class="text-red-600">Rp {{ number_format($row['sisa_akhir'], 0, ',', '.') }}</span>
                            @else

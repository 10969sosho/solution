            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                 ...
                 <div>
                     ...
                     @php($selectedGolonganIds = old('golongan_ids', $setting->golongans->pluck('id')->push($setting->golongan_id)->filter()->unique()->all()))
                     <div class="grid grid-cols-2 gap-2 border border-gray-300 rounded-lg p-3">
                         @foreach($golongans as $golongan)
                         <label class="flex items-center gap-2 text-sm">
                             <input type="checkbox" name="golongan_ids[]" value="{{ $golongan->id }}" {{ in_array($golongan->id, $selectedGolonganIds) ? 'checked' : '' }}>
                             {{ $golongan->name }}
                         </label>
                         @endforeach
                     </div>
                     <p class="text-xs text-gray-500 mt-1">Pilih beberapa golongan untuk satu jam kerja. Kosongkan untuk global.</p>
                     @error('golongan_ids') <p class="text-red-500 text-sm mt-1">{{ $message }}</p> @enderror
                 </div>
            </div>
            ...
            <div>
                ...
                <div class="grid grid-cols-4 gap-2">
                    ...
                    <label class="flex items-center px-3 py-2 border border-gray-300 rounded-lg cursor-pointer hover:border-blue-500">
                        <input type="checkbox" name="day[]" value="{{ $d }}" {{ in_array($d, (array) ($selectedDays ?? []), true) ? 'checked' : '' }}
                            class="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500">
                        <span class="ml-2 text-sm text-gray-700">{{ $d }}</span>
                    </label>
                    @endforeach
                </div>
                <p class="text-xs text-gray-500 mt-1">Pilih satu atau beberapa hari (Senin–Minggu). Kosongkan untuk berlaku setiap hari.</p>
            </div>

            ...
            <div class="border-t border-gray-200 pt-4 mt-2">
                ...
                <div class="grid grid-cols-2 gap-4 mb-4">
                    <div>
                        ...
                        <select name="payment_method" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                            ...
                            <option value="cash" {{ old('payment_method', $employee->payment_method) === 'cash' ? 'selected' : '' }}>Tunai (Cash)</option>
                        </select>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Nama Bank</label>
                        <input type="text" name="bank_name" value="{{ old('bank_name', $employee->bank_name) }}" placeholder="Contoh: BCA / Mandiri / BRI"
                            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                    </div>
                </div>
                ...
                <div class="grid grid-cols-2 gap-4 mb-4">
                    <div>
                        ...
                        <input type="text" name="account_number" value="{{ old('account_number', $employee->account_number) }}" placeholder="Contoh: 1234567890"
                            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Nama Pemilik Rekening</label>
                        <input type="text" name="account_holder" value="{{ old('account_holder', $employee->account_holder) }}" placeholder="Sesuai buku tabungan"
                            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                    </div>
                </div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="number" name="leave_quota" value="{{ old('leave_quota', $employee->leave_quota) }}" placeholder="Default: Admin/Mandor=2, AGK=1" min="0" max="31"
                            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                        <p class="text-[11px] text-gray-500 mt-1">Biarkan kosong untuk mengikuti aturan standar otomatis.</p>
                    </div>
                    <div class="flex items-center pt-6">
                        <label class="inline-flex items-center cursor-pointer">
                            <input type="checkbox" name="is_night_guard" value="1" {{ old('is_night_guard', $employee->is_night_guard) ? 'checked' : '' }} class="rounded border-gray-300 text-blue-600 shadow-sm focus:ring-blue-500 w-4 h-4">

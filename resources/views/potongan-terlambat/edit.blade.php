...
<div class="max-w-2xl mx-auto">
    <div class="bg-white rounded-xl shadow-sm">
        ...
        <form action="{{ route('potongan-terlambat.update', $potongan) }}" method="POST" class="p-6 space-y-4">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <select name="golongan_id" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                    ...
                    <option value="{{ $golongan->id }}" {{ old('golongan_id', $potongan->golongan_id) == $golongan->id ? 'selected' : '' }}>
                        {{ $golongan->name }}
                    </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <select name="type" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                    ...
                    <option value="masuk_kerja" {{ old('type', $potongan->type) == 'masuk_kerja' ? 'selected' : '' }}>Masuk Kerja</option>
                    <option value="setelah_istirahat" {{ old('type', $potongan->type) == 'setelah_istirahat' ? 'selected' : '' }}>Setelah Istirahat</option>
                </select>
            ...
            <div>
                ...
                <input type="text" id="amount" name="amount" value="{{ number_format(old('amount', $potongan->amount ?? 0), 0, ',', '.') }}" required
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                    placeholder="Contoh: 10.000">
                <input type="hidden" name="amount" id="amount_hidden" value="{{ old('amount', $potongan->amount ?? 0) }}">
